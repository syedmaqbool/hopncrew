...
function BrandPill({ icon }: { icon: any }) {
  return (
    <View style={styles.brandPill}>
      {typeof icon === 'string' ? (
        <Text style={{ color: '#111', fontWeight: '700', fontSize: 12 }}>{icon}</Text>
      ) : (
        <Image source={icon} style={{ width: 36, height: 36, maxWidth: '100%', resizeMode: 'contain' }} />
      )}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  picker: { height: 56, color: '#111', lineHeight: 0, borderRadius: 46, padding: 0, fontSize: 10, },
  ...
  squareLogo: {
    width: 26, height: 26, borderRadius: 6, backgroundColor: ' ',
    alignItems: 'center', justifyContent: 'center',
  },

  brandsRow: { gap: 8, paddingHorizontal: 16, paddingBottom: 10, marginTop: 16 },
  brandPill: {
    paddingHorizontal: 0, paddingVertical: 0, borderRadius: 10, backgroundColor: 'transparent', borderWidth: 0, borderColor: '#EEE',
  },
