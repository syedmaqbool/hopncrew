// src/screens/PassengerWaitModel.tsx
...
type Props = NativeStackScreenProps<RootStackParamList, 'PassengerWait'>;

const MINT = '#B9FBE7';
const SHEET_RATIO = 0.7; // 70% of the current window height

export default function PassengerWaitModel({ navigation }: Props) {
  const insets = useSafeAreaInsets();
  const { height } = useWindowDimensions();

  // Dynamic, rotation-aware sheet height (stays at bottom)
    ...
    <View style={StyleSheet.absoluteFill}>
      ...
      <SafeAreaView edges={['bottom']} style={styles.wrap}>
        <View style={[styles.sheet, { height: sheetHeight }]}>
          ...
          {/* Content scrolls; header/footer stay fixed within the sheet */}
          <ScrollView
          ...
          >
            <Text style={styles.h1}>Passenger Late Arrival & No Show</Text>
            ...
            <View style={styles.callout}>
              ...
              <Text style={[styles.calloutSub, { fontWeight: '800' }]}>
...
const styles = StyleSheet.create({
  ...
  noteTxt: { color: '#111', fontFamily: FONTS.bold },
});
