...
export type RideCalculatePayload = {
  ...
  stop_overs?: RidePoint[]; // keep for future
  passengers?: number;      // NEW
  luggage?: RideLuggageItem[]; // NEW
};
...
export type PriceBreakdown = {
  ...
  /** optional total distance fare and vehicles count if backend sends them */
  total_distance_fare?: number;
...
export type VehicleOption = {
  ...
  capacity_breakdown?: CapacityBreakdown; // NEW
  image_url?: string | null;
};
...
export async function calculateRideCost(
  ...
  const res = await api.post('/trips/calculate-trip-cost', payload, {
    validateStatus: () => true,
  });
  ...
  const request_parameters = data?.request_parameters ?? undefined;

  /** vehicle_options (defensive) */
  ...
  const vehicle_options: VehicleOption[] = optionsRaw.map((it) => ({
    ...
    /** NEW fields mirrored from backend if present */
    price_per_vehicle: it?.price_per_vehicle != null ? Number(it.price_per_vehicle) : undefined,
    vehicles_needed: it?.vehicles_needed != null ? Number(it.vehicles_needed) : undefined,
    ...
    price_breakdown: {
      ...
      total_distance_fare:
        it?.price_breakdown?.total_distance_fare != null
          ? Number(it.price_breakdown.total_distance_fare)
          : undefined,
      vehicles_needed:
        it?.price_breakdown?.vehicles_needed != null
          ? Number(it.price_breakdown.vehicles_needed)
          : undefined,
    },
    ...
    capacity_breakdown: it?.capacity_breakdown ?? undefined,
    image_url: it?.image_url ?? null,
  }));
...
export function vehicleOptionsToQuotes(vehicle_options: VehicleOption[]) {
  return (vehicle_options || []).map((v) => {
    ...
    return {
      ...
      eta: v.estimated_duration_minutes,
    };
  });
}